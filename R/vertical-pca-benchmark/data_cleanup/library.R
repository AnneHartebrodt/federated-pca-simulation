      ...
      for(s in nr_sites){
        for(e in eigenvector_update){
          for(q in qr_method){
            ...
            dir.name <- file.path(base_directory, o, v, s, e, q, g)
            ...
            for (f in which(str_detect(myfiles, paste0(suffix, '$')))) {
              ...
              data$qr_method<-q
              data$orthonormalisation_skip<-g
...
read_all_files_vector<-function(base_directory, suffix){
  data_list<-list()
  counter<-1
  for(o in data_orientation){
    for(v in vector_or_subspace)
        ...
        for(e in eigenvector_update){
          for(q in qr_method){
              ...
              for (f in which(str_detect(myfiles, paste0(suffix, '$')))) {
                ...
                data$eigenvector_update<-e
                data$qr_method<-q
                data$orthonormalisation_skip<-1
                data$filename<-file.path(dir.name, myfiles[f])
                head(data)
                data_list[[file.path(dir.name, myfiles[f])]]<-data
