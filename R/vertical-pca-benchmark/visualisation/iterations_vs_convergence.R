  ...
  chr1.plot<-ggplot(data[facet_title %in% paste('Eigenvector', c(1,2,5,7,10)) & test =='Chromosome 1'], aes(iterations, mean_value, col=as.factor(name_qr)))+
    ...
    facet_grid(test~facet_title, scales = 'free_x')+
 ...
 # chr1.plot
  ...
  mnist.plot<-ggplot(data[facet_title %in% paste('Eigenvector', c(1,2,5,7,10))& test =='MNIST'], aes(iterations, mean_value, col=as.factor(name_qr)))+
    geom_line(size=0.75)+
    theme_bw()+ylab('')+   
    xlab('iterations')+
    scale_color_manual(values = viridis(4)[1:3])+
