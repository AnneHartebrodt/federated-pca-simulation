 ...
 scales_x <- list(
  ...
  a<- annotate_figure(full.plot,
                  bottom = text_grob("iterations", size = 10,family = 'Arial'),
                  left = text_grob("angle w.r.t reference", rot = 90, size = 10, family = 'Arial'))
  #full.plot,
  ggsave(a, filename = outfile, width=10, height=10, units = 'cm', device = cairo_pdf)
