...
mat_file.100<-'~/Documents/featurecloud/pca/approximative-vertical/res-prelim/chr2.100000/vertical/matrix'
vec_file.100<-'~/Documents/featurecloud/pca/approximative-vertical/res-prelim/chr2.100000/vertical/vector'
res.100<-process_angles(mat_file = mat_file.100 , vec_file = vec_file.100, name = '100000')
convergence.100000<-res.100[[1]]

mat_file.500<-'~/Documents/featurecloud/pca/approximative-vertical/res-prelim/chr2.500000/vertical/matrix'
vec_file.500<-'~/Documents/featurecloud/pca/approximative-vertical/res-prelim/chr2.500000/vertical/vector'
res.500<-process_angles(mat_file = mat_file.500 , vec_file = vec_file.500, name = '500000')
convergence500000<-res.500[[1]]

mat_file.all<-'~/Documents/featurecloud/pca/approximative-vertical/res-prelim/chr2.all/vertical/matrix'
vec_file.all<-'~/Documents/featurecloud/pca/approximative-vertical/res-prelim/chr2.all/vertical/vector'
res.all<-process_angles(mat_file = mat_file.all , vec_file = vec_file.all, name = '1140556')
...
data$Features<- ordered(data$Features, levels=c('100000', '500000', '1140556'))
conv<-ggplot(data[sites ==5&eigenvector %in% c('Eigenvector 1', 'Eigenvector 5', 'Eigenvector 10')], 
  ...
  theme(legend.position = 'bottom',
        ...
        panel.spacing = unit(0.05, 'cm'),strip.background = element_blank() , strip.text.x = element_blank())+
...
conv
ggsave(conv,file='/home/anne/Documents/manuscripts/vertical-pattern-recognition/figures/chr2_convergence.pdf', height=8, units = 'cm', width = 15 )
...
time.all<-time_reader(mat_file.all, '1140556')
...
eo.all<-eo_reader(mat_file.all, vec_file.all, '1140556')
...
volume.all<-transmission_read(mat_file.all, vec_file.all, '1140556')
volume<-rbind(volume.100, volume.500, volume.all)

volume$name<- ordered(volume$name)
levels(volume$name)<-c('100000', '500000', '>1Mio')
############FAKE ALPHA LEGEND
...
# Convert the plot to a grob
gp
ggsave(gp,file='/home/anne/Documents/manuscripts/vertical-pattern-recognition/figures/chr2_benchmark.pdf', height=6, units = 'cm', width = 15)
