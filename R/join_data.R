...
for (k in c(2,5)){
  ...
  for(i in 1:20){
    ...
    for(f in fil){
      ...
      if(cl ==1){
      system(paste0("cat ", f, '/coding_only.tsv', ' > ', paste0(opt$out,'/', k, '_', i, '.tsv')))
      }else{
        system(paste0("tail -n ", nr_lines,' ', f, '/coding_only.tsv', ' >> ', paste0(opt$out,'/', k, '_', i, '.tsv')))
      }
      dal[[cl]]<-data.table(seq(1:nr_lines), rep(cl, nr_lines), basename(f))
      cl = cl+1
    }
    dal <-rbindlist(dal)
    fwrite(dal, file = paste0(opt$out, '/clusters/', k, '_', i, '_clusters.tsv'), sep='\t')
  }
