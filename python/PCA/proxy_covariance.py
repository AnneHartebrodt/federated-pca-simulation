...
def compute_noisy_cov(original, epsilon0=1, delta0=1, noise=False):
    """
...
def perform_SVD(noisy_cov, var_exp=0.5, ndims=100, mult_dims_returned=1):
    """
...
def aggregate_partial_SVDs(svd_list, intermediate_dims=None, ndim=100, weights=None):
    """
    ...
    if weights is not None:
        ...
        for svd in range(1, len(svd_list)):
            Ac = Ac + weights[svd] * (
                np.dot(svd_list[svd][0:intermediate_dims, :].transpose(), svd_list[svd][0:intermediate_dims, :]))
    else:
    ...
    UT = np.transpose(UT)
    UT = normalize_eigenvectors(UT)
    # self.logger.info('...done')
    return UT[:, 0:nd], S[0:nd]


def aggregate_partial_SVDs_balacan(svd_list, intermediate_dims=None, ndim=100, weights=None):
    """
    This function aggregates the local proxy covariances by averaging them
...
def local_PCA(original, epsilon0, delta0, noise=True, var_explained=0.5):
    noisy_cov = compute_noisy_cov(original, epsilon0, delta0, noise=noise)
    PC = perform_SVD(noisy_cov, var_exp=var_explained)
    return PC
...
def normalize_eigenspaces(svd_list):
...
def projection(scaled, sim, ndims, filename=None):
...
def standalone_pca(data, ndims=1000, noise=False, epsilon=1, delta=1, var_explained=0.5):
    """
    ...
    cov = compute_noisy_cov(data, noise=noise, epsilon0=epsilon, delta0=delta)
    # cov = (1 / (n - 1)) * sc.dot(data.transpose(), data)  # use unbiased estimator
    # print(cov[1:10,1])

    V_global, S, W, nd = svd_sub(cov, ndims)
