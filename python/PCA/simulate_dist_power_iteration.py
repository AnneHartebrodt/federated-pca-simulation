...
class AngleRunner():
    ...
    def unqeal_split(self, data, interval_end, scale_variance=True, center=True, scale01=False, scale_unit=False,
                     ndims=100, header=0, rownames=0, scale_var=True, transpose=False, sep='\t', exp_var=0.5,
                     mult_dims_ret=1, weights=None, balacan=False):
        """
        ...
        for i in range(len(interval_end)):
            ...
            pca, W1, E1 = self.simulation.run_standalone(data_sub, outfile, dims=ndims, header=header,
                                                         ...
                                                         scale01=scale01, scale_unit=scale_unit, transpose=transpose,
                                                         sep=sep, drop_samples=[], log=True, exp_var=exp_var)
        ...
        for mm in mult_dims_ret:
            ...
            if weights is not None:
                ...
                Xw.append(X)
            if balacan:
                W, X = self.ddppca.aggregate_partial_SVDs_balacan(Ac, ndim=ndims, intermediate_dims=mm)
                W = self.ddppca.normalize_eigenvectors(W)
                Wb.append(W)
                Xb.append(X)
        return Ws, Xs, Ww, Xw, Wb, Xb

    def run_and_compare_unequal(self, datafile, outfile=None, dims=100, header=0, rownames=0, center=True,
                                scale_var=True, scale01=False, scale_unit=False, transpose=False, sep='\t',
                                reported_angles=20, exp_var=0.5, mult_dims_ret=[1, 2, 1.5, 5]):

        # ID is the folder name
        study_id = path.basename(path.dirname(datafile))
        print('Standalone PCA before outlier removal')

        data, varnames, sampleids = self.importer.data_import(datafile, header=header, rownames=rownames,
                                                              transpose=False, sep=sep)
        n = data.shape[0]
        dims = min(dims, n)

        pca, W1, E1 = self.simulation.run_standalone(data, outfile, dims=dims, header=header, rownames=rownames,
        ...
        pca, W1, E1 = self.simulation.run_standalone(data, outfile, dims=dims, header=header, rownames=rownames,
                                                     ...
                                                     drop_samples=outliers, log=True, exp_var=exp_var)
        ...
        for ar in range(len(interval_end)):
            for i in range(10):
                ...
                Ws, Xs, Ww, Xw, Wb, Xb = self.unqeal_split(data, interval_end[ar], ndims=dims, exp_var=exp_var,
                                                           mult_dims_ret=mult_dims_ret, weights=perc[ar], balacan=True)
                for w in range(len(Ws)):
                    ...
                    with open(outfile + '/angles_unequal_splits_weighted_' + str(mult_dims_ret[w]) + '.tsv',
                              'a+') as handle:
                        handle.write(cv.collapse_array_to_string(angles, study_id=study_id))
                    with open(outfile + '/eigenvalues_weighted_' + str(mult_dims_ret[w]) + '.tsv', 'a+') as handle:
                        handle.write(cv.collapse_array_to_string(Xs[w][0:reported_angles], str(i)))

                    angles = co.compute_angles(W1, Wb[w], reported_angles=reported_angles)
                    with open(outfile + '/angles_unequal_splits_b_' + str(mult_dims_ret[w]) + '.tsv',
                              'a+') as handle:
                        handle.write(cv.collapse_array_to_string(angles, study_id=study_id))
                    with open(outfile + '/eigenvalues_b_' + str(mult_dims_ret[w]) + '.tsv', 'a+') as handle:
                        handle.write(cv.collapse_array_to_string(Xs[w][0:reported_angles], str(i)))
...
if __name__ == "__main__":
    ...
    outfile = '/home/anne/Documents/featurecloud/results/gexp_stats/mnsit/'
    exp_var = 0.5
    sep = ','
    ...
    sim.run_and_compare_unequal(inputfile, summaryfile, dims=dims, scale_unit=False, sep=sep, reported_angles=20,
                                exp_var=exp_var, mult_dims_ret=mult_dims_ret, rownames=None, header=0)
