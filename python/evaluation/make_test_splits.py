...
def split_mnist():
    ...
    data = scale_center_data_columnwise(data, center=True, scale_variance=False)
    data = data.T
...
def split_and_write(data, directory, name, colnames=None):
    ...
    for i in range(len(data_splits)):
        ...
        d.to_csv(os.path.join(directory, name) +'_split_randomized_rows_' + str(
                i) + '.tsv', header=False, index=False, sep='\t')
        if colnames is None:
            mycolnames = ['C' + str(x) for x in range(d.shape[1])]
        else:
...
def split_gwas(dirname,  inputfile, chr):
    ...
    data = data.values
    data = data.astype('float')
    split_and_write(data, dirname, chr)
...
if __name__ == '__main__':
    ...
    #filename = '/home/anne/Documents/featurecloud/pca/vertical-pca/data/1000g/raw/chr1/chr1.thin.traw.scaled'
    #split_gwas(dirname, inputfile=filename)

    #dirname = '/home/anne/Documents/featurecloud/dev/pca-tool/fed-pca-client/test_datasets/chr2'
    #filename = '/home/anne/Documents/featurecloud/pca/vertical-pca/data/1000g/raw/chr2/chr2.thin.traw.scaled'
    #split_gwas(dirname, inputfile=filename, chr='chr2')
